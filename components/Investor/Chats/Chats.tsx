...
import { getInvestorMessageData, getClickInvestorUserChatData, ContactUserByInvestor, ContactUserByInvestorWithShareFile, getAdminDataInvestor } from "../../../lib/investorapi";
...
export default function Chats(props: any) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      getClickInvestorUserChatData(data)
        .then(res => {
          if (res.status == true) {
  ...
  const fetchUserMessageDetails = async (id: any) => {
    try {
      ...
      const res = await getInvestorMessageData(data);
      if (res.status == true) {
        setActiveChat(0);
        setAdminChatSiderBar(res.userchatsider);
        setAdminChatMessage(res.userchatdata);
  ...
  const fetchAdminData = async () => {
    ...
    try {
      const res = await getAdminDataInvestor();
      if (res.status) {
  ...
  const imageChange = async (e: any) => {
    ...
    ContactUserByInvestorWithShareFile(formData)
      .then(res => {
        if (res.status === true) {
          setMessage('');
          fetchUserMessageDetails(currentUserData.id);
  ...
  return (
    <>
      <div className="userprofile-part" id='your-element-id1'>
